...
class Code(object):
    def __init__(
        ...
        power=1,
        noise_cov=1,
        pwr_func=nmd.empirical_variance_power_func,
        use_radius=True,
        **stim_kwargs
    ):
        if use_radius:
        ...
        self.encoding_matrix = self.make_encoding_matrix(
            n_neurs, self.stim_proc, power=power, pwr_compute=pwr_func
        )
    ...
    def get_min_distance_ns(self, eps=0.001, second=False):
    ...
    def get_representation(self, stim, noise=False):
        ...
        for i in range(stim.shape[1]):
            rep_i = np.array(
                list(self.rep_dict[self.stim_dict[tuple(s[i])]] for s in stim)
            )
            reps = reps + rep_i
        if noise:
    ...
    def compute_specific_ccgp(
        ...
        **dec_kwargs
        ...
        gen_stim1 = tuple(
            rs + gen_dist * (i == gen_dim) for i, rs in enumerate(ref_stim)
        )
        gen_stim2 = tuple(
            rs + gen_dist * (i == gen_dim) for i, rs in enumerate(tr_stim)
        )
    ...
    def compute_ccgp(self, use_vals=1, n_reps=10, **dec_kwargs):
        ...
        for i, (c1_train, c2_train) in enumerate(train_sets):
            ...
            pcorr[i] = self.decode_rep_classes(
                c1_train,
                c2_train,
                c1_test=c1_test,
                c2_test=c2_test,
    ...
    def _sample_noisy_reps(self, rs, n, add_noise=True, ret_stim=False, ref_stim=None):
    ...
    def _make_decoding_reps(self, c1, c2, n, add_noise=True, balance_training=False):
        n_half = int(n / 2)
        c1_train_reps, stim = self._sample_noisy_reps(
            c1, n_half, add_noise=add_noise, ret_stim=True
        )
        if balance_training:
            ref_stim = stim
        else:
            ref_stim = None
        c2_train_reps = self._sample_noisy_reps(
            c2, n_half, add_noise=add_noise, ref_stim=ref_stim
        )
        reps = np.concatenate((c1_train_reps, c2_train_reps), axis=0)
        labels = np.concatenate((np.zeros(n_half), np.ones(n_half)))
        return reps, labels

    def decode_rep_classes(
        self,
        c1,
        c2,
        n_train=10**3,
        ...
        **classifier_params
        ...
        for i in range(n_reps):
            ...
            if c2_test is None:
                c2_test = c2
            out = self._make_decoding_reps(
                c1_test, c2_test, n_test, add_noise=test_noise
            )
    ...
    def _get_partitions(self, random_thr=100, sample=None):
        ...
        if ss.comb(self.n_stimuli, class_size) > random_thr:
            partitions = list(
                np.random.choice(self.n_stimuli, class_size, replace=False)
    ...
    def compute_shattering(self, n_reps=5, thresh=0.75, **dec_args):
    ...
    def _get_rep_distances(self, reps, n_stim=1, dist_metric=l2_norm):
        stim_combs_mixes = np.array(list(it.combinations(self.stim, n_stim)))
        stim_combs_doubles = np.stack((self.stim,) * n_stim, axis=1)
        stim_combs = np.concatenate((stim_combs_mixes, stim_combs_doubles), axis=0)
    ...
    def get_average_swaps(self, n_stim, n=100, eps=0.001):
    ...
    def get_posterior_probability(self, reps, n_stim=1):
        return self._get_rep_distances(
            reps, n_stim=n_stim, dist_metric=self.noise_distr.logpdf
        )
    ...
    def compute_discrim(self, n_stim=1, **kwargs):
        if n_stim > 1:
            ...
            metric = hamming
        return self.compute_trl_metric(metric=metric, n_stim=n_stim, **kwargs)
...
class LinearCode(Code):
    ...
    def get_minimum_distance_theor(self):
        dl = np.sqrt(12*self.power/(self.n_feats*(self.n_values**2 - 1)))
        return dl
...
class CompositeCode(Code):
    def __init__(
        ...
        super().__init__(
            n_feats, n_values, n_neurs, power=total_power, noise_cov=noise_cov
        )

    def make_stimuli(self, n_feats, n_values):
        stim = self.codes[0].stim
        stim_proc = list(zip(*tuple(c.stim_proc for c in self.codes)))
        stim_dict = dict(zip(stim, stim_proc))
        return stim, stim_proc, stim_dict
