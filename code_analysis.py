...
def get_model_performance(
    ...
    **model_args
    ...
    for i, pwr in enumerate(pwrs):
        lc = model(
            n_feats, n_values, n_neurs, power=pwr, noise_cov=noise_var, **model_args
        )
...
def ccgp_pair_error_rate(d_nl, lg, lc1, lc2, noise_var, n_neurs, n=5000):
    dot_distr = sts.multivariate_normal(
        np.zeros_like(lg), 1 / n_neurs, allow_singular=True
    )
    dot_distr = sts.multivariate_normal(np.zeros_like(lg), 0, allow_singular=True)

    a = np.sqrt(lg**2 + 0.5 * d_nl**2 + np.sqrt(2) * d_nl * lg * dot_distr.rvs(n))
    ...
    d = (0.5 * lc1**2 + np.sqrt(2) * lc2 * d_nl * dot_distr.rvs(n)) / c
    # print((c2/2)**2, b2**2)
    # print((c2/2)**2 -  b2**2)
...
def find_necessary_power(
    ...
    include_flex=True,
    reg_weight=0,
    print_theory_diff=False,
):
    def opt_func(arg):
        p_l, p_n = arg
        gen = 1 - ccgp_error_rate(p_l, p_n, k, ni, noise_var, n_neurs, n=n)[0]
        flex = 1 - partition_error_rate(p_n, noise_var, ni**k)
        loss = (
            include_ccgp * np.abs(target - gen)
    ...
    p_l, p_n = res.x
    p_l_ana, p_n_ana = analytic_necessary_power(
        k, ni, noise_var, n_neurs, include_flex=include_flex, include_ccgp=include_ccgp
    )
    if print_theory_diff and res.fun > 0.0001:
        print("opt", p_l, p_n)
        print("ana", p_l_ana, p_n_ana)
        print("fun", res.fun)
    if print_theory_diff and (
        np.abs(p_l - p_l_ana) > 0.01 or np.abs(p_n - p_n_ana) > 0.01
    ):
        print("--- off ---")
        print("opt", p_l, p_n)
        print("ana", p_l_ana, p_n_ana)
...
def make_gen_flex_pwr_regions(reg_range, feat_range, val_range, *args, **kwargs):
    ...
    for i, reg in enumerate(reg_range):
        out = make_gen_flex_pwr(feat_range, val_range, *args, regions=reg, **kwargs)
        out_npwr[i], out_lpwr[i] = out
    return out_lpwr, out_npwr


def make_gen_flex_pwr(
    feat_range,
    val_range,
    noise_var,
    n_neurs,
    ...
    **kwargs
    ...
    for i, k in enumerate(k_reg):
        for j, ni in enumerate(val_range):
            if use_analytic:
                p_lopt, p_nopt = analytic_necessary_power(
                    k, ni, noise_var, n_neurs, **kwargs
                )
            else:
                p_lopt, p_nopt = find_necessary_power(
                    k, ni, noise_var, n_neurs, n=n, **kwargs
                )
...
def make_ccgp_shatter_phase(
    pwr, n_trades, feat_range, val_range, noise_var, n_neurs, n=5000
):
    trades = np.linspace(0, 1, n_trades)
    combs = list(it.product(trades, feat_range, val_range))
    combs_inds = list(
        it.product(range(n_trades), range(len(feat_range)), range(len(val_range)))
    )
    ...
    for i, (t, k, ni) in enumerate(combs):
        p_l = pwr * (1 - t)
        p_n = pwr * t
        ccgp[combs_inds[i]] = ccgp_error_rate(p_l, p_n, k, ni, noise_var, n_neurs, n=n)[
            0
        ]
...
def ccgp_error_rate(p_l, p_n, n_feats, n_values, noise_var=1, n_neurs=1000, n=5000):
...
def compute_composite_swap_distance_empirical(
    pwrs, n_feats, n_values, tradeoff=0.5, n_neurs=1000, n_stim=2, n_samps=1000
):
...
def compute_composite_distance_theory(
    pwrs, n_feats, n_values, tradeoff=0.5, n_neurs=1000, n_samples=1000, supply_ds=None
):
    pwrs = np.array(pwrs)
    if len(pwrs.shape) == 0:
...
def _stim_swaps(stim):
    ...
    u_swaps = set()
    for r in rotations:
        r_stim = np.stack(list(np.roll(stim[:, i], r[i]) for i in range(k)), axis=1)
...
def compute_composite_hamming_theory(
    pwrs,
    n_feats,
    n_values,
    noise_var=1,
    ...
    **kwargs
    ...
    out = compute_composite_distance_theory(
        pwrs, n_feats, n_values, tradeoff=tradeoff, **kwargs
    )
...
def compute_composite_hamming_empirical(
    pwrs,
    n_feats,
    n_values,
    n_neurs=1000,
    ...
    **kwargs
    ...
    for i, pwr in enumerate(pwrs):
        code = cc.make_code(tradeoff, pwr, n_feats, n_values, n_neurs, **kwargs)
...
def compute_composite_ccgp_empirical(
    pwrs,
    n_feats,
    n_values,
    n_neurs=1000,
...
def _compute_metrics(
    ...
    n_neurs,
    trade,
    compute_shattering=True,
    compute_discrim=True,
    compute_mse=True,
    trade_reps=10,
    total_power=10,
    noise_cov=1,
    **compute_kwargs
    ...
    for j in range(trade_reps):
        ...
        if compute_shattering:
            ...
            p_scores = sd[2]
            if j == 0:
                p_perfs = np.zeros((trade_reps,) + p_scores.shape)
                shatt = np.zeros((trade_reps,) + sd[0].shape)
            p_perfs[j] = p_scores
            shatt[j] = sd[0]
        ...
        if compute_discrim:
            ...
            if j == 0:
                discrim_errs = np.zeros((trade_reps,) + discrim_err.shape)
            discrim_errs[j] = discrim_err

        if compute_mse:
            mse_err = c.compute_mse()
            if j == 0:
                mse_errs = np.zeros((trade_reps,) + mse_err.shape)
            mse_errs[j] = mse_err
...
def _merge_parallel_output(out, compute_shattering, compute_discrim, compute_mse):
    ccgps = []
    total_dist = []
    indiv_dists = []
    p_perfs = []
    ...
    info = {
        "min_dist_full": np.array(total_dist),
...
def get_ccgp_dim_tradeoff(
    ...
    **compute_kwargs
    ...
    cm = ft.partial(
        ...
        compute_discrim=compute_discrim,
        **compute_kwargs
    )
    with mp.Pool() as p:
        out = p.map(cm, trades)
    metrics, info = _merge_parallel_output(
        out, compute_shattering, compute_discrim, compute_mse
    )
    return trades, metrics, info


def get_multiple_tradeoffs(
    snrs,
    n_trades,
    n_feats,
    n_values,
    ...
    **compute_kwargs
    ...
    for snr, n_feat, n_val, n_neur in arg_combs:
        ...
        errs, t1, t2 = ccgp_error_rate(
            p_l, p_n, n_feats, n_values, noise_var, n_neur, n=10000
        )
